...
router.get('/pettyCash/:parentExpenseId',verify,(request, response) => {
  ...
  pool
  .query('SELECT sfid, Project_Name__c FROM salesforce.Milestone1_Expense__c WHERE  sfid = $1',[parentExpenseId])
  .then((expenseQueryResult) => {
      console.log('expenseQueryResultabctest  : '+JSON.stringify(expenseQueryResult.rows));
      if(expenseQueryResult.rowCount > 0)
      {
        ...
        .query('Select sfid , Name FROM salesforce.Activity_Code__c where Project__c = $1', [projectId])
         .then((activityCodeQueryResult) => {
          ...
          if(activityCodeQueryResult.rowCount > 0)
          {
            ...
            response.render('expenses/pettyCash/pettycash',{objUser, parentExpenseId:parentExpenseId, lstActivityCode:lstActivityCode });
          }
          else
          {
...
router.post('/conveyanceform',(request,response) => {  
  ...
  /*  const schema=joi.object({
   ...
   if(result.error)
   {
    ...
    let conveyanceVoucherInsertQuery = format('INSERT INTO salesforce.Conveyance_Voucher__c (From__c, To__c,Project_Task__c, Purpose_of_Travel__c,Mode_of_Conveyance__c,Kms_Travelled__c,Amount__c,Heroku_Image_URL__c, Expense__c) VALUES %L returning id', lstConveyance);
...
router.get('/fetchActivityCodes', verify,async (request, response) => {
  ...
  console.log('parentId :' +parentExpenseId)
  let projectId= '';
  console.log('expenseId  : '+parentExpenseId);
  ...
  .then((milestoneQueryResult) => {
      ...
      if(milestoneQueryResult.rowCount > 0)
      {
          ...
          let activityQuery = 'Select sfid , Name FROM salesforce.Activity_Code__c where Project__c = $1';
          console.log('activityQuery  : '+activityQuery);
          pool
          .query(activityQuery, projectId)
          .then((activityQueryResult) => {
            console.log('activityQueryResult: '+JSON.stringify(activityQueryResult.rows));
                if(activityQueryResult.rowCount > 0)
                {
                  response.send(activityQueryResult.rows);
                }
          })
          .catch((taskQueryError) => {
                console.log('taskQueryError  : '+taskQueryError.stack);
